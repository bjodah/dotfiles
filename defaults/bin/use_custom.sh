...
#  - rel: CFLAGS="-march=native -O3 -ffast-math" LDFLAGS=-Wl,-rpath-link,/opt/openblas-0.3.10/lib CC=gcc-10 cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_INSTALL_PREFIX=/opt/sundials-5.3.0-klu-openblas -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON -DBUILD_STATIC_LIBS=OFF -DEXAMPLES_ENABLE_C=ON -DEXAMPLES_INSTALL=ON -DOPENMP_ENABLE=OFF -DLAPACK_ENABLE:BOOL=ON -DKLU_ENABLE:BOOL=ON -DKLU_INCLUDE_DIR=/usr/include/suitesparse -DKLU_LIBRARY_DIR=/usr/lib/x86_64-linux-gnu -DLAPACK_LIBRARIES=/opt/openblas-0.3.10/lib/libopenblas.so -DSUNDIALS_INDEX_SIZE=32 -DCMAKE_INSTALL_RPATH="/opt/sundials-5.3.0-klu-openblas/lib;/opt/openblas-0.3.10/lib" -DCMAKE_BUILD_WITH_INSTALL_RPATH=ON ../sundials-5.3.0/
...
# ccls:
#  $ git clone --depth=1 --recursive https://github.com/MaskRay/ccls && mkdir ccls/build && cd ccls/build && cmake -DCMAKE_BUILD_TYPE=Release -DCLANG_RESOURCE_DIR=$(clang-10 -print-resource-dir) -DCMAKE_INSTALL_PREFIX=/opt/ccls .. && make -j 8 && make install

# pycvodes2: CC=gcc-10 PYCVODES2_LAPACK=openblas CFLAGS="-O3 -march=native -isystem /usr/include/suitesparse -isystem /opt/sundials-5.3.0-klu-openblas/include" LDFLAGS="-Wl,--disable-new-dtags -Wl,-rpath,/opt/sundials-5.3.0-klu-openblas/lib -L/opt/sundials-5.3.0-klu-openblas/lib" python3 setup.py develop --user
